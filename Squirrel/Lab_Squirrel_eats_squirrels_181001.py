...
CAMERASLACK_HORIZONTAL = 60
CAMERASLACK_VERTICTAL = 50
...
def runGame():
    ...
    while True: # main game loop
        ...
        #TUKA JA ZAMENUVAME CAMERASLACK PROMENLIVATA SO CAMERASLACK_HORIZONTAL I CAMERASLACK_VERTICAL SOODVETNO
        #levo/desno
        if (camerax + HALF_WINWIDTH) - playerCenterx > CAMERASLACK_HORIZONTAL:
            ...
            camerax = playerCenterx - CAMERASLACK_HORIZONTAL - HALF_WINWIDTH
        #gore/dolu
        if (cameray + HALF_WINHEIGHT) - playerCentery > CAMERASLACK_VERTICTAL:
            cameray = playerCentery + CAMERASLACK_VERTICTAL - HALF_WINHEIGHT
        elif playerCentery - (cameray + HALF_WINHEIGHT) > CAMERASLACK_VERTICTAL:
            cameray = playerCentery - CAMERASLACK_VERTICTAL - HALF_WINHEIGHT
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        #4. Change the logic of the game. If the squirrel is hit by the larger animal
                        # it becomes smaller using the same logic as for the getting bigger at the current game.
                        # The game is over when the squirrel grows to the WINSIZE or LOSTSIZE (5).
...
def makeNewSquirrel(camerax, cameray):
    ...
    #3. Introduce the possibility an enemy squirrels to bounce downwards
    # (the squirrel could bounce only in one direction only upwards or only downwards).
    # The direction is determined when creating an enemy squirrel. (3 points)
    if direction < 0:
